...
class AppImagePackager(CollectionPackagerBase):
    ...
    def createPackage(self):
        ...
        if not self.internalCreatePackage(
            defines, packageSymbols=True, seperateSymbolFiles=True
        ):
            return False
        ...
        if etc.exists():
            if not utils.createSymlink(
                etc, archiveDir / "etc", useAbsolutePath=False, targetIsDirectory=True
            ):
                return False
        if "runenv" in defines:
            ...
            with (archiveDir / "apprun-hooks/craft-runenv-hook.sh").open("wt") as hook:
                ...
                hook.writelines(
                    [
                        f"{x}={y.join('=')}\nexport {x}"
                        for x, y in [i.split("=") for i in defines["runenv"]]
                    ]
                )
        if not utils.createDir(self.packageDestinationDir()):
            return False
        desktopFiles = glob.glob(
            f"{archiveDir}/usr/share/applications/*{defines['appname']}.desktop"
        )
        ...
        with utils.ScopedEnv(env):
            ...
            if CraftCore.debug.verbose() > 0:
                args += ["-v0"]
            return utils.system(
                ["linuxdeploy-x86_64.AppImage"] + args, cwd=self.packageDestinationDir()
            )
