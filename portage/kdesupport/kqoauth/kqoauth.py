    ...
    def install(self):
        ...
        for file in os.listdir(os.path.join(self.sourceDir(), "src")):
            if file.endswith(("kqoauthrequest.h", "kqoauthrequest_1.h", "kqoauthrequest_xauth.h", "kqoauthmanager.h",
                              "kqoauthglobals.h")):
                utils.copyFile(os.path.join(self.sourceDir(), "src", file),
                               os.path.join(self.installDir(), "include", "QtKOAuth", file))
        utils.copyFile(os.path.join(self.sourceDir(), "include", "QtKOAuth"),
                       os.path.join(self.installDir(), "include", "QtKOAuth", "QtKOAuth"))
        utils.copyFile(os.path.join(self.sourceDir(), "kqoauth.prf"),
                       os.path.join(self.installDir(), "mkspecs", "features", "kqoauth.prf"))
        return True
