...
class subinfo(info.infoclass):
    ...
    def setTargets( self ):
      ...
      self.patchToApply['0.4.1'] = ("phonon-backend-vlc-0.4.1-20111223.diff", 1)
...
class Package( CMakePackageBase ):
    def __init__( self, **args ):
        ...
        self.subinfo.options.configure.defines = '-DPHONON_BUILDSYSTEM_DIR=\"%s;%s\" ' % (os.path.join(os.getenv('KDEROOT'),'share','phonon','buildsystem').replace('\\','/'),os.path.join(os.getenv('KDEROOT'),'share','phonon-buildsystem').replace('\\','/'))
