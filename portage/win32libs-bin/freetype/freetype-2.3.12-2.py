...
class subinfo( info.infoclass ):
    def setTargets( self ):
        ...
        for version in [ '2.3.7-2', '2.3.12-2' ]:
            self.targets[ version ]          = self.getPackage( repoUrl, 'freetype', version )
            self.targetDigestUrls[ version ] = self.getPackage( repoUrl, 'freetype', version , '.tar.bz2.sha1' )

        self.defaultTarget = '2.3.12-2'
