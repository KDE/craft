...
class subinfo(info.infoclass):
    ...
    def setTargets( self ):
        ...
        for ver in utils.utilsCache.getNightlyVersionsFromUrl(vlcBaseUrl, r"\d\d\d\d\d\d\d\d-\d\d\d\d"):
            ...
            self.targets[ vlcTagName +"-debug-git" ]  = "%s/vlc-%s-%s-git-win%s-debug.7z" % (vlcBaseUrl, vlcTagName, ver, vlcArch  )
...
class Package(BinaryPackageBase):
    ...
    def install( self ):
        ...
        if not (int(ver2[0]) >= 2 and int(ver2[0]) >= 1):
            utils.copyFile( os.path.join( self.installDir() , "lib" ,"libvlc.dll.a" ) , os.path.join( self.installDir() , "lib" ,"libvlc.lib" ))
            utils.copyFile( os.path.join( self.installDir() , "lib" ,"libvlccore.dll.a" ) , os.path.join( self.installDir() , "lib" ,"libvlccore.lib" ))
