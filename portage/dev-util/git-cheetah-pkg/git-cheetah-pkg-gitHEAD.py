...
class Package( AutoToolsPackageBase):
    def __init__( self ):
        ...
        utils.putenv("PATH","%s;%s" % ( os.getenv("PATH") , os.path.join( os.getenv("KDEROOT") , "dev-utils" , "git" , "bin" )))
    ...
    def qmerge(self):
        if not AutoToolsPackageBase.qmerge(self):
            return False
        utils.system("regsvr32 -s -n -i:machine %s" % os.path.join(self.mergeDestinationDir() , "bin" , "git_shell_ext.dll" ))
        return True
        
    def unmerge(self):
        utils.system("regsvr32 -u -s -n -i:machine %s" % os.path.join(self.mergeDestinationDir() , "bin" , "git_shell_ext.dll" ))
